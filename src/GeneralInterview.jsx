...
export default function GeneralInterview() {
  ...
  const parse = async () => { if (!url.trim()) return; const id = crypto.randomUUID(); const source = { id, url: url.trim(), title: "待确认面经来源", platform: "链接收件", capturedAt: new Date().toISOString(), credibility: "待评估", parseStatus: sourceText.trim() ? "preview" : "saved" }; await dbPut("interviewSources", source); const lines = sourceText.split(/\n+/).map(x => x.replace(/^[-*\d.、)]+\s*/, "").trim()).filter(x => x.length > 8); const candidates = (lines.length ? lines : ["请介绍一个与该岗位相关的项目，并说明你的具体贡献。", "面试官可能会如何追问你的数据、业务或协作决策？"]).map((text, i) => ({ id: `${id}-${i}`, scope: "general", text, category: /sql|python|数据|技术/i.test(text) ? "technical" : /介绍|项目|经历/i.test(text) ? "behavior" : "case", categories: [], sourceIds: [id], jobIds: [], starred: false, favorite: false, practicedAt: "", answer: "", difficulty: "待评估" })); setPreview(candidates); setSourceMsg(sourceText.trim() ? "已解析，请预览并勾选后合并" : "已保存来源；未提供正文时，请先打开链接复制面经文字到下方"); };
  const merge = async () => { for (const q of preview.filter(x => x.selected !== false)) { await dbPut("interviewQuestions", q); await dbPut("interviewPrep", q); } setItems(xs => [...xs, ...preview.filter(x => x.selected !== false)]); setPreview([]); setSourceText(""); setUrl(""); setSourceMsg("已合并到 General 题库"); };
  return <section className="general-interview panel">
    ...
    <div className="source-inbox"><div className="source-title"><strong>面经链接收件箱</strong><span>先解析预览，确认后才进入题库</span></div><input value={url} onChange={e=>setUrl(e.target.value)} placeholder="粘贴面经链接"/><textarea value={sourceText} onChange={e=>setSourceText(e.target.value)} placeholder="可选：粘贴链接页面中的面经文字（受登录/跨域限制时使用）"/><div className="source-actions"><button className="secondary" onClick={parse}>解析并预览</button>{preview.length>0&&<button className="primary" onClick={merge}>确认合并 {preview.length} 题</button>}</div>{sourceMsg&&<small>{sourceMsg}</small>}{preview.length>0&&<div className="preview-list">{preview.map((q,i)=><label key={q.id}><input type="checkbox" checked={q.selected!==false} onChange={e=>setPreview(xs=>xs.map((x,j)=>j===i?{...x,selected:e.target.checked}:x))}/><span>{q.text}</span></label>)}</div>}</div>
