...
function Settings({userId,onImported}){const legacy=previewLegacy();const [message,setMessage]=React.useState("");const migrate=async()=>{const n=await importLegacy(userId);setMessage(`已导入 ${n} 条，旧数据仍保留在本机`);onImported()};return <><Header title="设置" subtitle="私有云、旧版迁移和扩展连接。"/><div className="page settings-grid"><section className="panel"><p className="eyebrow">CLOUD</p><h2>Supabase 私有同步</h2><p>当前：{cloudEnabled?"环境已配置":"本机预览模式"}。生产环境通过邮箱白名单和 RLS 限制为单用户。</p></section><section className="panel"><p className="eyebrow">MIGRATION</p><h2>旧版数据迁移</h2><p>检测到 {legacy.length} 条可迁移记录。按岗位 URL 去重，成功前不会删除旧数据。</p><button className="secondary" disabled={!legacy.length} onClick={migrate}><Icons.UploadCloud/>预览并导入</button>{message&&<p className="success-text">{message}</p>}</section><section className="panel"><p className="eyebrow">EXTENSION</p><h2>浏览器网页伴侣</h2><p>负责抓取 JD、同步岗位和辅助填表；扩展没有最终提交接口。</p></section></div></>}
