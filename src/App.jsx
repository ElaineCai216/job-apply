...
function Interview({jobs,onSelect}){const list=jobs.filter(j=>j.stage==="interview"||j.outcome==="面试");return <><Header title="面试准备" subtitle="先练通用能力；收到真实面试后，再增加岗位专属题库。"/><div className="page"><GeneralInterview/><section className="panel role-interviews"><div className="panel-head"><h2>岗位专属面经</h2><Badge>{list.length} 个面试</Badge></div><JobTable jobs={list} onSelect={onSelect} empty="收到面试后，定制面经和深挖题会集中在这里"/></section></div></>}
