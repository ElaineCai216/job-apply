...
修复恢复密钥仅保存在单一设备时，换机或重装后无法继续同步的问题。

## 实现

- 增加 `sync_spaces`，将不可解密的历史密文标记为非活动旧空间；不删除、不覆盖。
- 新空间使用新的设备端 AES-256-GCM 密钥；该密钥再由用户独立恢复口令经 PBKDF2-SHA-256（310,000 次）派生的密钥加密后保存为恢复包。
- 新设备可用恢复口令解锁活动空间；已解锁设备仍可短暂显示密钥以完成直接迁移。
- 岗位记录与加密材料按 `sync_space_id` 隔离，旧空间不会被拉取或影响新的跨端同步。
- 同步健康页对空间查询、下载同步与云端校验设定 12 秒超时；请求悬挂时恢复可操作状态并展示错误。

## 验证

- `npm test`：12 项通过（含恢复包加密、解密与错误口令回归）。
- `npm run build`：通过。
- 数据库迁移待部署后进行设备端安装与实际同步验证。

## 风险与回滚

- 忘记恢复口令且所有设备密钥都丢失时，新空间同样无法解密；因此界面仍提供一次性恢复密钥显示和加密备份。
- 迁移只扩展表与主键，不删除旧行；可将 App 回滚到上一版本，但上一版本不识别新空间字段。
