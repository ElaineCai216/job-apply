...
Deno.serve(async request => {
  ...
  if (payload.mode === "enroll") {
    ...
    const { data, error } = await supabase.from("collector_devices").insert({ user_id: user.id, name: clean(payload.name, 80) || "Safari on Mac", token_hash: await digest(token) }).select("id,expires_at").single();
  ...
  const { data: device } = await supabase.from("collector_devices").select("id,user_id").eq("token_hash", await digest(rawToken)).is("revoked_at", null).gt("expires_at", new Date().toISOString()).maybeSingle();
  if (!device) return Response.json({ error: "Collector token expired" }, { status: 401, headers: cors });
  if (payload.mode === "session-expired") {
    await supabase.from("jobsdb_source_inbox").insert({ user_id: device.user_id, canonical_url: `jobsdb-session:${new Date().toISOString()}`, source_page: clean(payload.sourcePage, 500), status: "session_expired" });
  ...
  const rows = jobs.map((job: Record<string, unknown>) => ({ user_id: device.user_id, canonical_url: canonical(clean(job.url, 1800)), company: clean(job.company, 240), role: clean(job.role, 300), location: clean(job.location, 240), jd: clean(job.jd, 30000), source_page: clean(job.sourcePage, 1800), status: "pending" })).filter((job: { canonical_url: string; role: string }) => /^https:\/\//.test(job.canonical_url) && Boolean(job.role));
  if (rows.length) await supabase.from("jobsdb_source_inbox").upsert(rows, { onConflict: "user_id,canonical_url", ignoreDuplicates: true });
  await supabase.from("collector_devices").update({ last_used_at: new Date().toISOString() }).eq("id", device.id);
